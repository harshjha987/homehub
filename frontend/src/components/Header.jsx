...
function Header() {
  ...
  return (
    <header className='h-16 w-full bg-gradient-to-b from-gray-800 to-gray-900 flex items-center shadow-md justify-between bg-transparent p-5'>
      <Link to="/">
        <h1 className='font-bold text-xl md:text-2xl flex flex-wrap md:px-20'>
      ...
      <form onSubmit={handleSubmit} className='bg-slate-100 p-3 rounded-lg flex items-center'>
        <input
          type='text'
          placeholder='Search...'
          className='bg-transparent focus:outline-none w-24 md:w-56'
          value={searchTerm}
      ...
      <ul className="flex flex-wrap md:px-20">
        <Link to="/">
          <li className="text-slate-100 mx-3 hidden sm:inline hover:underline cursor-pointer">Home</li>
        </Link>
        ...
        <Link to="/contact">
          <li className="text-slate-100 mx-3 hidden sm:inline hover:underline cursor-pointer">Contact Us</li>
        </Link>

        <Link to="/profile">
          {currentUser ? (
            <img className="rounded-lg w-7 h-7 object-cover" src={currentUser.avatar} alt="profile" />
          ) : (
